// ════════════════════════════════════════════════════════════════
// FILE 2: layouts/Footer.jsx  — replace your existing Footer.jsx
// ════════════════════════════════════════════════════════════════
...
export default function Footer() {
  ...
  const phone        = info?.phone        || "+977 98XXXXXXXX";
  const email        = info?.email        || "support@medisuite.com";
  const openingHours = info?.openingHours || "9:00 AM – 7:00 PM";
  const description  = info?.description  || "Pharmacy essentials and skincare you can trust.";
  const instagram    = info?.instagramUrl || "#";
  const facebook     = info?.facebookUrl  || "#";
  const newsletter   = info?.newsletterEnabled !== false;
  const nlText       = info?.newsletterText || "Get offers + skincare tips";
  const termsUrl     = info?.termsUrl     || "/pages/terms";
  const privacyUrl   = info?.privacyUrl   || "/pages/privacy";
  const refundUrl    = info?.refundUrl    || "/pages/refund";
      ...
      <div className="container py-5">
        <div className="row g-4">
          ...
          <div className="col-12 col-lg-4">
            <Link className="text-decoration-none d-flex align-items-center gap-2" to="/">
              <span className="d-inline-flex align-items-center justify-content-center rounded-circle">
                <img src={logo} alt="Medi Suite" style={{ width: 32, height: 32, objectFit: "contain" }} />
              </span>
            ...
            <p className="text-secondary mt-3 mb-3" style={{ maxWidth: 360 }}>{description}</p>

            <div className="d-flex gap-2">
              {instagram && instagram !== "" && (
                <a className="btn btn-outline-secondary rounded-pill btn-sm px-3" href={instagram} aria-label="Instagram" target="_blank" rel="noreferrer">
                  <i className="bi bi-instagram me-2" />Instagram
                </a>
              )}
              {facebook && facebook !== "" && (
                <a className="btn btn-outline-secondary rounded-pill btn-sm px-3" href={facebook} aria-label="Facebook" target="_blank" rel="noreferrer">
          ...
          <div className="col-12 col-lg-4">
            ...
            <div className="card card-soft p-3 mb-3">
              <div className="d-flex flex-wrap gap-3">
                ...
                <div>
                  ...
                  <div className="fw-semibold">{openingHours}</div>
                </div>
              </div>
            </div>
            ...
            {newsletter && (
              <>
                <div className="fw-bold mb-2">{nlText}</div>
                <form className="d-flex gap-2">
                  <input className="form-control rounded-pill" placeholder="Enter your email" type="email" />
                  <button className="btn btn-brand rounded-pill px-4" type="button">Subscribe</button>
                </form>
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2">
          <div className="small text-secondary">© {year} Medisuite. All rights reserved.</div>
          <div className="d-flex flex-wrap gap-3">
            <Link className="small text-decoration-none text-secondary" to={termsUrl}>Terms</Link>
            <Link className="small text-decoration-none text-secondary" to={privacyUrl}>Privacy</Link>
            <Link className="small text-decoration-none text-secondary" to={refundUrl}>Refund policy</Link>
          </div>
        </div>
      </div>
    </footer>
